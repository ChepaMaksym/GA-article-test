...
fprintf(fid, 'Generated with profile `%s`. This checklist maps article-level requirements to the active implementation.\n\n', result.profile);
fprintf(fid, '| Article operator / claim | Local function | Verification status |\n');
fprintf(fid, '|---|---|---|\n');
fprintf(fid, '| Chromosome `[x,y,Q,H]` | `article_exact_spec`, optimizer population rows | Specification and bounds are encoded directly |\n');
fprintf(fid, '| MGA fixed `Pc/Pm` | `nmga_options`, `run_evolutionary_optimizer` | Article values are used by the MGA path |\n');
fprintf(fid, '| NMGA Formula 7 `Pc=P1*(1-gen/maxgen)^b` | `nmga_adaptive_rates` | Implemented as the generation schedule |\n');
fprintf(fid, '| NMGA Formula 8 `Pm=P2*(1+gen/maxgen)^b` | `nmga_adaptive_rates` | Implemented as the generation schedule |\n');
fprintf(fid, '| AGP/EGP split | `article_gene_pool_split` | Split count is derived from article `beta` |\n');
fprintf(fid, '| AGP/EGP inheritance branch | `article_egp_crossover`, `breed_nmga` | Implemented in the NMGA breeding path |\n');
fprintf(fid, '| Nonuniform mutation amplitude decreases | `article_nonuniform_delta`, `mutate_nonuniform` | Generation-dependent amplitude is implemented |\n');
fprintf(fid, '| Article SSE objective / reciprocal fitness detail | `source_objective` | Article SSE mode is selected by the protocol |\n');
fprintf(fid, '| Bounds and population-size preservation | `run_evolutionary_optimizer` | Enforced by initialization and bounded variation |\n');
fprintf(fid, '| Synthetic data provenance | `run_article_exact_reproduction_impl` | Generated reports explicitly mark `synthetic_reproduction` |\n\n');
