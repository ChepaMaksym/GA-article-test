...
PROFILES = ("OLD", "HYBRID_RESET", "HYBRID_NO_RESET")
...
def write_svg(path: Path, old: list[int], reset: list[int], no_reset: list[int]) -> None:
    width, height, margin = 900, 520, 70
    all_values = old + reset + no_reset
    ymax = max(all_values) * 1.05
    ymin = min(all_values) * 0.95
    span = max(1.0, ymax - ymin)

    def xy(index: int, value: int) -> tuple[float, float]:
        x = margin + index / (len(old) - 1) * (width - 2 * margin)
        y = height - margin - (value - ymin) / span * (height - 2 * margin)
        return x, y

    groups = [(old, "OLD"), (reset, "HYBRID_RESET"), (no_reset, "HYBRID_NO_RESET")]
    polylines = []
    labels = []
    dash = ["", " stroke-dasharray=\"8,4\"", " stroke-dasharray=\"2,5\""]
    for idx, (values, label) in enumerate(groups):
        points = " ".join(f"{x:.2f},{y:.2f}" for x, y in (xy(i, v) for i, v in enumerate(values)))
        polylines.append(f'<polyline fill="none" stroke="black" stroke-width="1.8"{dash[idx]} points="{points}"/>')
        labels.append(f'<text x="{width-250}" y="{45 + idx*24}" font-family="sans-serif" font-size="14">{label}</text>')
    svg = f'''<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">
<rect width="100%" height="100%" fill="white"/>
<text x="{width/2}" y="28" text-anchor="middle" font-family="sans-serif" font-size="19">EU26-27 paired FE to Pareto completion</text>
<line x1="{margin}" y1="{height-margin}" x2="{width-margin}" y2="{height-margin}" stroke="black"/>
<line x1="{margin}" y1="{margin}" x2="{margin}" y2="{height-margin}" stroke="black"/>
{''.join(polylines)}
{''.join(labels)}
<text x="{width/2}" y="{height-18}" text-anchor="middle" font-family="sans-serif" font-size="14">paired seed index</text>
...
def main() -> int:
    ...
    for run_index, seed in enumerate(seeds, start=1):
        ...
        for profile in PROFILES:
            value = parse_single_endpoint(root / profile / f"seed-{seed}")
            endpoints[profile].append(value)
            record[profile] = value
        rows.append(record)

    h1_values = paired_relative_reductions(endpoints["OLD"], endpoints["HYBRID_RESET"])
    h1_ci = bootstrap_median_ci(h1_values)
    h1_median = statistics.median(h1_values)
    h1_status = "PASS" if h1_ci[0] > 0 else "FAIL"

    h2_values = paired_relative_reductions(endpoints["HYBRID_NO_RESET"], endpoints["HYBRID_RESET"])
    ...
    if h2_ci[0] > 0:
        ...
        h2_status = "NO_CLEAR_EFFECT"

    report = {
        "schema": "eu26-27-hybrid-confirmatory-v1",
        "seeds": seeds,
        "complete_runs": {profile: len(endpoints[profile]) for profile in PROFILES},
        ...
        "H1": {
            "statistic": "paired median relative FE reduction OLD -> HYBRID_RESET",
            "median": h1_median,
        ...
        "H2": {
            "statistic": "paired median relative FE reduction HYBRID_NO_RESET -> HYBRID_RESET",
            "median": h2_median,
    ...
    write_svg(out / "paired_fe.svg", endpoints["OLD"], endpoints["HYBRID_RESET"], endpoints["HYBRID_NO_RESET"])
    summary = f'''# EU26-27 HYBRID confirmatory result

```text
OLD median FE: {statistics.median(endpoints['OLD'])}
HYBRID_RESET median FE: {statistics.median(endpoints['HYBRID_RESET'])}
HYBRID_NO_RESET median FE: {statistics.median(endpoints['HYBRID_NO_RESET'])}
H1 median relative reduction: {h1_median:.6%}
H1 95% bootstrap CI: [{h1_ci[0]:.6%}, {h1_ci[1]:.6%}]
H1: {h1_status}
H2 reset effect median: {h2_median:.6%}
