...
def run_two_rate_old(seed:int,*,n:int=100,offspring:int=10,max_evaluations:int=2_000_000,record_trace:bool=False)->OldRunResult:
    ...
    while len(archive)<n+1 and evaluations+offspring<=max_evaluations:
        ...
        best=max(x[0] for x in candidates); tied=[x for x in candidates if x[0]==best]; winner=tied[int(rng.integers(0,len(tied)))][2]; low+=winner=="lower"; high+=winner=="higher"; rate=update_two_rate(rate,winner,float(rng.random()),n)
