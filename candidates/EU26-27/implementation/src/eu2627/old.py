...
def update_two_rate(rate: float, winner_group: str, draw: float, n: int) -> float:
    ...
    return max(rate/2.0,0.5) if draw < p else min(rate*2.0,n/4.0)
...
def run_two_rate_old(seed: int, *, n: int=100, offspring: int=10, max_evaluations: int=2_000_000, record_trace: bool=False) -> OldRunResult:
    ...
    rng=np.random.Generator(np.random.PCG64DXSM(int(seed))); initial=random_bits(rng,n); w=oneminmax_objective(initial,n)[0]
    archive={w:initial}; first=[None]*(n+1); first[w]=1; evaluations=1; generations=0; rate=1.0; low=high=draws=flipped=0; insertions=1; trace=sha256(f"seed={seed};initial={initial};weight={w}".encode())
    while len(archive)<n+1 and evaluations+offspring<=max_evaluations:
        weights=tuple(sorted(archive)); bits=tuple(archive[x] for x in weights); candidates=[]; half=offspring//2
        for i in range(offspring):
            parent=bits[int(rng.integers(0,len(bits)))]; group="lower" if i<half else "higher"; p=rate/(2*n) if group=="lower" else 2*rate/n
            child,l=conditional_mutation(parent,n,p,rng); draws+=1; flipped+=l; weight=oneminmax_objective(child,n)[0]; c=hypervolume_contribution(weights,weight,n); candidates.append((c,weight,group,child)); evaluations+=1
            if first[weight] is None: first[weight]=evaluations
        best=max(x[0] for x in candidates); tied=[x for x in candidates if x[0]==best]; winner=tied[int(rng.integers(0,len(tied)))][2]
        if winner=="lower": low+=1
        else: high+=1
        rate=update_two_rate(rate,winner,float(rng.random()),n)
        for _,weight,_,child in candidates:
            if weight not in archive: archive[weight]=child; insertions+=1
        generations+=1
        if record_trace: trace.update(f";g={generations};size={len(archive)};winner={winner};rate={format(rate,'.17g')}".encode())
    effort=EffortLedger(evaluations,generations*offspring,draws,flipped,insertions,generations); effort.validate(offspring)
    digest=sha256(",".join(map(str,sorted(archive))).encode()).hexdigest()
