...
## Уточнена мета від 10 вересня 2026 року

Наступний етап присвячено відтворенню й розмежуванню застою,
локального максимуму та насичення λ з подальшою перевіркою механізмів
виходу. Перевага методу не задається як обов'язковий результат.
Почати читання слід з [дослідницької мети](RESEARCH_OBJECTIVE_UK.md)
та [аудиту поточної реалізації](IMPLEMENTATION_AUDIT_2026-09-10_UK.md).
Теоретичні результати та їх обмеження зібрано в
[огляді літератури 2010–2026 років](LOCAL_OPTIMA_REVIEW_UK.md).

Новий протокол і серію ще не виконано. Наведені нижче статуси
стосуються завершених експериментів і не змінюються через уточнення
мети. Історичний Jump-контроль задає локальний максимум явно;
гармонізований Census-експеримент його наявності не перевіряв.

Програмне продовження від 23 вересня:
[перевірюване покоління та журнал оцінювань](local_optima/README.md).
Воно усуває повторний вибір мутанта в окремому компоненті, не змінюючи
історичного коду. Повного пошуку й нових наукових результатів тут ще немає.
...
Це результати історичної реалізації. Медіани NFE обчислено для різних
підгруп успішних запусків, тому 32,1% не є загальною оцінкою прискорення.
Аудит від 10 вересня виявив повторний вибір мутанта за нічиєї в старому
ядрі; наслідки для нового протоколу наведено в окремому аудиті вище.
