...
GitHub role authenticity is not inferable from a profile JSON document. A
separately retrieved API provenance record is mandatory for formal H5.
"""
...
def _validate_h2(value: Any, head: str, expected_payload_digest: str) -> str:
    ...
    if status == "NOT_EVALUATED_MATLAB_REPORT_ABSENT":
        ...
        return status
    if status not in {
        "PASS_CROSS_ENV_FIXED_TAPE",
        "NOT_EVALUATED_UNAUTHENTICATED_EXTERNAL_MATLAB_REPORT",
    }:
        raise EvidenceValidationError("profile contains a failed or unauthenticated H2 gate")
    ...
    if status == "PASS_CROSS_ENV_FIXED_TAPE":
        if h2["report_origin"] != "RUNNER_INVOKED_ENGINE":
            raise EvidenceValidationError("formal H2 was not generated by the profile runner")
    ...
    if status == "PASS_CROSS_ENV_FIXED_TAPE" and (
        h2["exact_mismatches"] != [] or h2["numeric_mismatches"] != []
    ):
        raise EvidenceValidationError("H2 pass retains mismatches")
...
def validate_profile(
    ...
    expected_profile_status = (
        "PASS_PROFILE_H0_H4"
        if h2_status == "PASS_CROSS_ENV_FIXED_TAPE"
...
def _validate_api_provenance(
    ...
    try:
        ...
        return True, "API_METADATA_BINDING_VALID_EXTERNAL_AUTHENTICITY_ASSUMED"
...
def compare_reports(
    ...
    *,
    ...
    required_roles["github_actions_4core"] = bool(
        required_roles["github_actions_4core"] and github_api_binding_valid
    )

    profile_gate_failures = [
        profile["profile_label"]
        for profile in validated.values()
        if profile["profile_status"] != "PASS_PROFILE_H0_H4"
    ]
    if not strict_pairs_pass:
        h5_status = "INCONCLUSIVE_STRICT_MISMATCH"
    elif github_profile is not None and not github_api_binding_valid:
        h5_status = "INCOMPLETE_UNAUTHENTICATED_GITHUB_PROFILE"
    elif not all(required_roles.values()):
        h5_status = "INCOMPLETE_REQUIRED_PROFILE"
    elif profile_gate_failures:
        h5_status = "INCOMPLETE_PROFILE_GATE"
    else:
        h5_status = "PASS_FORMULA_PORTABILITY"
    overall_status = (
        "PASS_ENGINEERING_FORMULA_PORTABILITY"
        if h5_status == "PASS_FORMULA_PORTABILITY"
